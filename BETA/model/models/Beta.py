...
import random
import pathlib
from model.lib.tabpfn.utils import load_model_workflow
...
class Beta(nn.Module):
    ...
    *,
    ...
    model_string='',
        ...
        if d_num == 0:
            ...
            self.num_module = make_module(
                num_embeddings, n_features=d_num
            )
        ...
        self.device=device
        self.style = None
        self.TabPFN, self.c, self.results_file = load_model_workflow(0, 42, add_name=model_string, base_path=base_path, device=device,
                                                         eval_addition='', only_inference=True)
    ...
    def train_step(self, x_num, x_cat, y, batch_size):
        ...
        candidate_indices = [i for i in range(num_samples)]# if i not in train_indices]
        ...
        logits = self.forward(
            ...
            x_num_candidate, x_cat_candidate,
            y_candidate, is_train=True
        )

        return logits, y_train

    def forward(self, x_num, x_cat, 
                candidate_x_num, candidate_x_cat, candidate_y, is_train=False,is_val=False,is_test=False,indexs=None):
        ...
        if is_train:
            ...
            candidate_x_cat_sample = torch.tensor(candidate_x_cat[indices], device=self.device) if candidate_x_cat is not None else None
            candidate_x_num_sample = torch.tensor(candidate_x_num[indices], device=self.device).float() if candidate_x_num is not None else None
            candidate_y_sample = torch.tensor(candidate_y[indices], device=self.device).long() if candidate_y is not None else None
            x_num = torch.tensor(x_num, device=self.device).float() if x_num is not None else None
            x_cat = torch.tensor(x_cat, device=self.device)if x_cat is not None else None
            x = []
            candidate_x = []
            if x_num is not None:
                x_num_sample = x_num
                x.append(x_num_sample if self.num_module is None else self.num_module(x_num_sample))
                candidate_x.append(candidate_x_num_sample if self.num_module is None else self.num_module(candidate_x_num_sample))
            if x_cat is None:
            ...
            if self.k is not None:
                ...
                candidate_x = candidate_x[:, None].expand(-1, self.k, -1)  # (B, D) -> (B, K, D)
            ...
            with autocast():
                logits=self.TabPFN((None, input, y_input), single_eval_pos=y_input.shape[0]-x.shape[0])[:, :, :self.d_out].to(torch.float)
            # print(logits.dtype)
        else:
            val_logits = []
            x_num = torch.tensor(x_num, device=self.device).float() if x_num is not None else None
            ...
            for k in range(self.k):
                    if indexs is None:
                        indices = np.random.randint(0, candidate_size, (min(1000,candidate_size),))
                    ...
                    val_logits.append(self.TabPFN((None, input, y_input), single_eval_pos=y_input.shape[0]-x.shape[0])[:, :, :self.d_out])
