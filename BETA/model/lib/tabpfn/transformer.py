...
class TransformerModel(nn.Module):
    def __init__(self, encoder, n_out, ninp, nhead, nhid, nlayers, dropout=0.0, style_encoder=None, y_encoder=None,
        ...
        # encoder_layer_creator = lambda: layer.FlashAttentionTransformerEncoderLayer(ninp, nhead, nhid, dropout, activation=activation,
        #                                                         pre_norm=pre_norm, recompute_attn=recompute_attn)
    ...
    def forward(self, src, src_mask=None, single_eval_pos=None):
        ...
        # print(train_x.dtype)
        src = torch.cat([global_src, style_src, train_x, x_src[single_eval_pos:]], 0).to(train_x.dtype)
        ...
        # print(output.shape)
        # print(output[single_eval_pos+len(style_src)+(self.global_att_embeddings.num_embeddings if self.global_att_embeddings else 0):])
