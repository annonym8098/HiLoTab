...
class RegressionEmbedding(nn.Module):
    def __init__(self, num_features, emsize):
        super().__init__()
        self.num_features = num_features
        self.emsize = emsize
        self.layer = nn.Parameter(torch.ones(num_features, emsize))


    def forward(self, x):
        x = self.layer[None] * x[:, :, None].squeeze(-2)
        return x
...
def get_Regression(num_features, emsize=512):
    return RegressionEmbedding(num_features, emsize)