...
class LitProtoGate(pl.LightningModule):
    ...
    def compute_loss(self, x: torch.Tensor, y_true: torch.Tensor):
        ...
        y_pred, neighbour_list, y_prob = proto_predict(x_selected, x_neighbour_selected, y_neighbour, self.pred_k)
        losses = self.compute_pred_loss(x_selected, x_neighbour_selected, y_true, y_neighbour)

        return  y_pred, losses, neighbour_list, y_prob
    ...
    def log_epoch_metrics(self, outputs, key, dataloader_name=""):
        ...
        y_prob = torch.cat([out["y_prob"].detach().cpu() for out in outputs], dim=0).numpy()
    
        acc = accuracy_score(y_true, y_pred)
        self.log(f'{key}/accuracy{dataloader_name}', acc)
        self.log(f'{key}/balanced_accuracy{dataloader_name}', balanced_accuracy_score(y_true, y_pred))
        self.log(f'{key}/F1_weighted{dataloader_name}', f1_score(y_true, y_pred, average='macro'))
        self.log(f'{key}/precision_weighted{dataloader_name}', precision_score(y_true, y_pred, average='macro'))
        self.log(f'{key}/recall_weighted{dataloader_name}', recall_score(y_true, y_pred, average='macro'))
        ...
        try:
            ...
            self.log(f'{key}/AUROC_weighted{dataloader_name}', auc)
        except ValueError:
            self.log(f'{key}/AUROC_weighted{dataloader_name}', 0.0)
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_pred, losses, _, y_prob = self.compute_loss(x, y_true)
    ...
    def test_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        y_pred, losses, neighbour_list, y_prob = self.compute_loss(x, y_true)
    ...
    def test_epoch_end(self, outputs):
        ...
        if self.feature_selection == True:
            ...
            for gate in gate_all:
                ...
                gate_bin_all.append(gate_binary_str)
...
def proto_predict(query, neighbors, neighbor_labels, k):
    ...
    probs = label_counts / label_counts.sum(dim=1, keepdim=True).clamp_min(1e-12)
    prediction = torch.argmax(probs, dim=1)

    return prediction, indices[:, :k], probs
