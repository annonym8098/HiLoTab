...
class LSPINClassifier(nn.Module):
    def __init__(
        ...
        for h in gate_hidden_dims:
            ...
            gate_layers.append(self._get_activation(activation_gate))
            prev_dim = h
...
class LLSPINClassifier(LSPINClassifier):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def forward(self, x, z=None, train_gates=True, compute_sim=True):
        return super().forward(x, z=z, train_gates=train_gates, compute_sim=compute_sim)

    def compute_loss(self, logits, labels, gate, alpha, z=None, compute_sim=True):
        return super().compute_loss(logits, labels, gate, alpha, z=z, compute_sim=compute_sim)
...
def train_model(model, X_train, y_train, params, device, seed):
    ...
    drop_last = len(X_train_tensor) >= params["batch_size"]
    ...
    for epoch in range(params["epochs"]):
        for xb, yb in train_loader:
            ...
            logits, gate, alpha = model(xb, z=xb, train_gates=True, compute_sim=True)
            loss, _, _, _ = model.compute_loss(logits, yb, gate, alpha, z=xb, compute_sim=True)
...
def predict_model(model, X_test, device, batch_size=1024):
    ...
    with torch.no_grad():
        for (xb,) in loader:
            xb = xb.to(device)
            logits, _, _ = model(xb, z=None, train_gates=False, compute_sim=False)
